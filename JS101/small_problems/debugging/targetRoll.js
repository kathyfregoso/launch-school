...
function targetRoll(characterValue) {
  ...
  console.log("--> " + result);
  // this code was missing the break statement, which resulted in multiple statements being executed
  switch (result) {
    case 1:
      automaticFail();
      break; // added
...
/* 
When playing around with the above program, our two test rolls result in two random values 
that produce the sample output below (because each dice roll produces a random value, 
  your output may differ). The outcome of rolling 16 looks correct, but the output when we 
  rolled values 1 and 20 doesn't make sense. For each roll, only one outcome should be displayed. 
